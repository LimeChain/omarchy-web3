...
- The guarded Anchor command injects `--ignore-keys`, `--no-idl`, the pinned SBF platform-tools version, and `--skip-tools-install`; init, test, localnet, deploy, keys, account, airdrop, and transaction operations are not exposed.
