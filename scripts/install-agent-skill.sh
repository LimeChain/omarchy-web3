...
if [[ ${LCW3_TESTING:-0} == 1 ]]; then
  ...
  echo "install-agent-skill: LCW3_HOME overrides are test-only" >&2
  exit 1
fi
...
python3 "$SCRIPT_DIR/managed-tree.py" install \
  --source "$SOURCE_DIR/skill/limechain-web3" \
  --target "$skill_root" \
  --boundary "$skill_home" \
  --scope agent-skill

echo "Optional agent skill installed at $skill_root"
echo "This opt-in component adds behavioral instructions to compatible coding agents."
